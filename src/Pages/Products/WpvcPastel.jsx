...
function WpvcPastel() {
    ...
    useEffect(() => {
      function handleClickOutside(event) {
        if (selectedProduct && !event.target.closest('.zoom-popup')) {
          ...
          {selectedProduct && (
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
              <div className="zoom-popup relative bg-gray-300 w-[300px] md:w-[500px] p-4 md:p-6 rounded-lg shadow-4xl border">
                <img src={selectedProduct.zoomImage} alt="Large Product" className="w-[300px] md:w-[500px] h-auto object-cover transition-transform duration-300 scale-105" />
                <div className="flex flex-col sm:flex-row items-center sm:items-start">
                  <img
                    src={AdhamsWhite}
                    alt="Large Product"
                    className="md:w-auto md:h-[100px] md:mt-12 h-[50px] w-auto mt-3 object-cover"
                  />
                  <div className="ml-12 md:mt-16">
                    <h1 className=" text-sm sm:text-lg">{selectedProduct.content}</h1>
                    <h1 className=" text-sm sm:text-lg">{selectedProduct.Description}</h1>
                    <h1 className=" text-sm sm:text-lg">{selectedProduct.size}</h1>
                  </div>
                </div>
                {/* Close Button */}
                <button
